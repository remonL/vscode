...
export class BrowserSocketFactory implements ISocketFactory {
	...
	connect(host: string, port: number, query: string, debugLabel: string, callback: IConnectCallback): void {
		...
		const socket = this._webSocketFactory.create(`${webSocketSchema}://${/:/.test(window.location.hostname) ? `[${window.location.hostname}]` : window.location.hostname}${window.location.port ? `:${window.location.port}` : ''}/?${query}&skipWebSocketFrames=false`, debugLabel);
